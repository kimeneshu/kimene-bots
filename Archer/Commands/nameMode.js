...
if(!data) new registerData({guildID: message.guild.id, nameMode: false}).save();
    switch (args[0]) {
        ...
        case "kapat":
            ...
            break;
        default:
            return message.channel.send(embed.setImage("https://cdn.discordapp.com/attachments/814077098426892288/896350512423792641/unknown.png").setDescription(`İsim modu açık iken kayıt mesajında geçmiş isimler görüntülenir kapalı iken görüntülenmez \n \`\`\`${ayar.bot.botPrefix}${this.config.name} aç/kapat\`\`\` `)).sil(20);
            break;
    }
};
exports.config = {
