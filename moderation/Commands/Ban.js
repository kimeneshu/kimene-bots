...
module.exports.run = async(client, message, args, embed) => {
        if (ayar.roles.banStaff.some(r => !message.member.roles.cache.has(r)) && !message.member.hasPermission(128)) return message.channel.send(embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)).sil(7);
    ...
    if (!member || !sebep) return message.channel.send(embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}ban @Kimene/ID [Reason]**`)).sil(7)
...
exports.config = {
    name: "ban",
    usage: `${ayar.bot.botPrefix}ban [@Kimene/ID] [reason]`,
    guildOnly: true,
    aliases: ["allah","ban","kimene","sg","anasınısik"],
    cooldown: 3000
};
