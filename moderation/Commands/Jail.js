...
module.exports.run = async(client, message, args, embed) => {
    if (ayar.roles.jailStaff.some(s => !message.member.roles.cache.has(s)) && !message.member.hasPermission(128)) return message.channel.send(embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)).sil(7);
    let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
    let sebep = args.slice(1).join(' ')
    if (!member || !sebep) return message.channel.send(embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}jail @Kimene/ID [Reason]**`)).sil(7)
    ...
    if (limitData && limitData.Count >= ayar.guild.jailLimit) return message.channel.send(embed.setDescription(`${message.member}, Bu günlük jail limit sınırına ulaştın.`)).sil(7)
...
exports.config = {
    name: "jail",
    usage: `${ayar.bot.botPrefix}jail [@Kimene/ID] [Reason]`,
    guildOnly: true,
