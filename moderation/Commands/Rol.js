...
module.exports.run = async(client, message, args, embed) => {
   if (!message.member.hasPermission(128)) return message.channel.send(embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)).sil(7);
    ...
    if(message.member.roles.highest.rawPosition <= role.rawPosition) return message.channel.send(embed.setDescription(`${message.member}, Kendinizden Üst pozisyondaki rolleri vermezsin uyanık seni`)).sil(7);
...
exports.config = {
    name: "rol",
    usage: `${ayar.bot.botPrefix}rol [@Kimene/ID] [@Role/ID/Name]`,
    guildOnly: true,
    aliases: ["r"],
    cooldown: 3000
};
