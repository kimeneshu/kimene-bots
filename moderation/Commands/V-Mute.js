...
module.exports.run = async(client, message, args, embed) => {
    if (ayar.roles.vmuteStaff.some(s => !message.member.roles.cache.has(s)) && !message.member.hasPermission(128)) return message.channel.send(embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)).sil(7);
    let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
    let zaman = args[1];
    let sebep = args.slice(2).join(' ')
    if (!member || !zaman || !sebep) return message.channel.send(embed.setDescription(`${message.member}, Hatalı kullanım. Örnek: **${ayar.bot.botPrefix}vmute @Kimene/ID [Time] [Reason]**`)).sil(7)
...
exports.config = {
    name: "vmute",
    usage: `${ayar.bot.botPrefix}vmute [@Kimene128/ID] [Time] [Reason]`,
    guildOnly: true,
