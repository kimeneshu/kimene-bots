...
`)).sil(7);
        ...
        let aktifunsesyt = rol.filter(s =>!s.user.bot &&  s.presence.status != 'offline' && !s.voice.channel)
        message.channel.send(embed.setDescription(`${aktifunsesyt.size} Yetkiliye ses çağrısı yapılıyor.`).setColor('RANDOM')).then(async(msg) => {
            aktifunsesyt.array().forEach(async(cross, index) => {
                setTimeout(async() => {
                    ...
                    cross.send(`Rica etsem ses kanalına girermisin özelden sana bizzat yazdım sakın kızma biz bir aileyiz <3 müsait degilsen seste afk bırakabilirsin..\n**${message.guild.name}**`).catch(err => message.channel.send(`${cross} Aktifsin fakat seste değilsin lütfen rica etsek ses kanalına girermisin. müsait degilsen afk bırakabilirsin.`) && msg.edit(embed().setDescription(`${cross} kullanıcısına özelden mesaj gönderilemediği için kanala etiketlendi.`)))
                }, index * 1500)
            })
